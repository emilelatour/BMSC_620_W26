...
| $x$      | 1    | 2    | 3    | 4    | 5    | 6    |
|----------|------|------|------|------|------|------|
| $P(X=x)$ | 0.10 | 0.20 | 0.05 | 0.05 | 0.25 | 0.35 |

. . .

$$\begin{aligned}
E(X) &= 1(0.10) + 2(0.20) + 3(0.05) + 4(0.05) + 5(0.25) + 6(0.35) \\
&= 0.10 + 0.40 + 0.15 + 0.20 + 1.25 + 2.10 \\
&= 4.20
\end{aligned}$$

. . .

The die is "loaded" toward higher values (mean is 4.20 vs. 3.5 for fair die)
...
&= (-2.5)^2 \cdot \frac{1}{6} + (-1.5)^2 \cdot \frac{1}{6} + (-0.5)^2 \cdot \frac{1}{6} \\
&\quad + (0.5)^2 \cdot \frac{1}{6} + (1.5)^2 \cdot \frac{1}{6} + (2.5)^2 \cdot \frac{1}{6} \\
...
If $X \sim \text{Binomial}(n, p)$, the probability of exactly $k$ successes is:
...
$$= \frac{10!}{4! \cdot 6!} (0.203)^4 (0.797)^6 = 210 \times 0.00170 \times 0.245 \approx 0.087$$
...
Result: About 86% chance that 3 or fewer have diabetes
...
Result: About 2.4% chance that 5 or more have diabetes
...
**Quick check with our COVID example:**

-   $X = 1$ (vaccinated) with $p = 0.25$
-   $E(X) = 0.25$ ✓
-   $\text{Var}(X) = 0.25(0.75) = 0.1875$ ✓

## From Bernoulli to Binomial

What if we repeat a Bernoulli trial multiple times?

**Example:** Flip a coin 10 times. How many heads do we get?

-   Each flip: Bernoulli(0.5)
-   Total number of heads: Binomial(10, 0.5)
