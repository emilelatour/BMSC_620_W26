...
subtitle: "Key Concepts from Lessons 1–9"
author: "Emile Latour"
...
- Question has "**less than**" or "**at most**" → `lower.tail = TRUE` (default)
- Question has "**greater than**" or "**at least**" → `lower.tail = FALSE`
...
This applies to **binomial and Poisson** (discrete). For Normal (continuous), $P(X \ge 5) = P(X \gt 5)$.
:::

**Question:** What is $P(X \ge 5)$ for a binomial distribution?

**Translate to computer language:** "At least 5" = "5 or more" = "greater than 4"
$$P(X \ge 5) = P(X \gt 4)$$

**Code it:**
```{r}
#| eval: false
pbinom(q = 4, size = 10, prob = 0.5, lower.tail = FALSE)  # Correct!
1 - pbinom(q = 4, size = 10, prob = 0.5)                  # Also works
```

**Key point:** For discrete distributions, "$X \ge 5$" equals "$X \gt 4$" because we can't have 4.5 successes!
...
upp_lbl <- glue::glue("pnorm({lim}, {mu}, {sigma})\nP(X > {lim}) = {upp_lbl}")
...
2. Larger samples → smaller standard error → more precise estimates
...
1. **Increase sample size** ($n$ ↑ → SE ↓ → narrower CI)
2. **Decrease confidence level** (95% → 90% → smaller $t^*$ → narrower CI)
    - But this means less confidence!
3. **Reduce variability** ($s$ ↓ → SE ↓ → narrower CI)
