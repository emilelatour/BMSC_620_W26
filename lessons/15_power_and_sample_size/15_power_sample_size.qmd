...
tibble(
  ...
  label = c("Descriptive\nStatistics", 
            ...
            "Confidence\nIntervals",
            "Hypothesis\nTests"),
  current = c(FALSE, FALSE, FALSE, FALSE, FALSE)
) %>%
  ggplot(aes(x = x, y = y)) +
  geom_segment(aes(x = x - 0.3, xend = x + 0.3, y = y, yend = y), 
               linewidth = 10, color = "#0072B2", alpha = 0.7) +
  geom_text(aes(label = label), size = 5, color = "white", fontface = "bold") +
  annotate("segment", x = 1.3, y = 1, xend = 1.7, yend = 1,
           arrow = arrow(length = unit(0.3, "cm")), linewidth = 1) +
  annotate("segment", x = 2.3, y = 1, xend = 2.7, yend = 1,
           arrow = arrow(length = unit(0.3, "cm")), linewidth = 1) +
  annotate("segment", x = 3.3, y = 1, xend = 3.7, yend = 1,
           arrow = arrow(length = unit(0.3, "cm")), linewidth = 1) +
  annotate("segment", x = 4.3, y = 1, xend = 4.7, yend = 1,
           arrow = arrow(length = unit(0.3, "cm")), linewidth = 1) +
  annotate("segment", x = 5.3, y = 1, xend = 5.7, yend = 1,
           arrow = arrow(length = unit(0.3, "cm")), linewidth = 1, color = "#D55E00") +
  annotate("text", x = 6, y = 1, label = "Power &\nSample Size", 
           size = 5.5, fontface = "bold", color = "#D55E00") +
  coord_cartesian(xlim = c(0.5, 6.5), ylim = c(0.5, 1.5)) +
  theme_void()
```

\

**Today we answer:** How do we design studies to have adequate power to detect effects?

**Why this matters:** No point running an experiment if it's unlikely to detect a real effect!

## Review: What we've learned about hypothesis tests

**We've covered three types of t-tests:**
...
-   Test stat: $t = \frac{\bar{x}_d - 0}{s_d/\sqrt{n}}$
:::
...
-   Test stat: $t = \frac{(\bar{x}_1-\bar{x}_2) - 0}{\sqrt{\frac{s_1^2}{n_1} + \frac{s_2^2}{n_2}}}$
:::
:::::

\

**Today's question:** For each of these tests, how do we determine if we have enough data to detect a real effect?
...
**Results we saw:** The caffeine group had significantly higher taps/min

\

**Questions we might ask *before* running this study:**

-   How many participants do we need?
-   What if we could only recruit 20 per group - would that be enough?
-   What factors determine whether we can detect an effect?
