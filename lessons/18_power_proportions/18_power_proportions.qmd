...
|---|---|---|
| **Before: Correct** | ***a*** — Concordant (+/+) | ***b*** — Discordant (+/−) |
| **Before: Incorrect** | ***c*** — Discordant (−/+) | ***d*** — Concordant (−/−) |
...
The **concordant pairs** (*a* and *d*, where before = after) give us no information about change.

Only the **discordant pairs** (*b* and *c*) tell us something changed — and McNemar's test focuses entirely on those.
:::

:::
:::::

## Notation for paired 2×2 tables
...
|  | **Post: +** | **Post: −** |
|---|:---:|:---:|
| **Pre: +** | ***a*** = $p_{11}$ (concordant) | ***b*** = $p_{12}$ (discordant) |
| **Pre: −** | ***c*** = $p_{21}$ (discordant) | ***d*** = $p_{22}$ (concordant) |

\

- **Concordant pairs** (*a* and *d*): outcome is the same at both time points — no information about change
- **Discordant pairs** (*b* and *c*): outcome changes — this is where the action is
  - ***b*** ($p_{12}$): changed from **+** to **−** (e.g., "got worse")
  - ***c*** ($p_{21}$): changed from **−** to **+** (e.g., "improved")
...
McNemar's test asks: among the discordant pairs, is the split between *b* and *c* different from 50/50?
:::
...
- *b* = pairs where outcome changed from **+ → −** (e.g., correct → incorrect)
- *c* = pairs where outcome changed from **− → +** (e.g., incorrect → correct)


::: {.callout-important icon="false"}
### Hypotheses

$H_0$: The probability of changing in each direction is equal ($b = c$)

$H_A$: The probability of changing differs by direction ($b \neq c$)
:::
...
1. **Total sample size** ($n$) — total number of matched pairs
2. **Proportion of discordant pairs** ($p_d = b + c$) — pairs where the outcome changes
3. **The direction of discordance** — which way is the change expected to go?
...
## Estimating power for McNemar's: a simplified approach (1/2)
...
- Proportion of pairs with outcome change: 
  - $p_d = b + c$ (total discordant pairs)
- Among discordant pairs, the proportion "improving" ($c$): 
  - $\phi = c / p_d$
- Under $H_0$: $\phi = 0.50$ (changes equally likely in both directions)



## Estimating power for McNemar's: a simplified approach (2/2)

**Example**: 

- 25% of pairs expected to be discordant
...
# Among discordant pairs, 70% are "improvements"
phi_alt <- 0.70
```

\

**Step 2:** Solve for the number of discordant pairs needed
...
This tells us we need **`r ceiling(mcnemar_power$n)` discordant pairs**. 

\

::: {.callout-tip icon="false"}
But we can't just enroll discordant pairs — we have to enroll *everyone* and see who changes.
:::

## From discordant pairs to total sample size

\

**Step 3:** Convert to total pairs needed

- Take the calculated number of discordant pairs needed
- Divide by the expected proportion of pairs to be discordant
